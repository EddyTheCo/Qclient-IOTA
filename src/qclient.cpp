...
Client::Client(QObject *parent) : QObject(parent), nam(new QNetworkAccessManager(this)), m_state(Disconnected)
{
    connect(this, &Client::last_blockid, this, [=](qblocks::c_array id) { qDebug() << id.toHexString(); });
};
...
Response *Client::get_reply_rest(const QString &path, const QString &query)
{
    ...
    if (!JWT.isNull())
        request.setRawHeader(QByteArray("Authorization"), QByteArray(("Bearer " + JWT).toUtf8()));
...
Response *Client::post_reply_rest(const QString &path, const QJsonObject &payload)
{
    ...
    request.setAttribute(QNetworkRequest::UseCredentialsAttribute, false);
    if (!JWT.isNull())
        request.setRawHeader(QByteArray("Authorization"), QByteArray("Bearer ").append(JWT.toUtf8()));
    request.setHeader(QNetworkRequest::ContentTypeHeader, "application/json");
